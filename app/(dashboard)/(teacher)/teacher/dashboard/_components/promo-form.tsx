...
import { createOrUpdatePromo, isUniquePromoCode } from '@/lib/actions/promo.action';
...
const formSchema = z.object({
  ...
  code: z.string().min(6).max(12).toUpperCase().refine(async (code) => {
    return await isUniquePromoCode(code)
  }, {message: "Promo code already exists"}),
  discount: z.number().min(1).max(100),
  expiresAt: z.date().optional().refine((date) => {
...
export function PromoForm({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      console.log(values)
      toast.success(`Promo Code ${type == "edit"? "updated": "created"} successfully`)
      setIsCreating(false)
      router.refresh()
    } catch {
              ...
              <FormItem>
                <FormControl>
                  ...
                  {!field.value ? (
                    ...
                    <span className="text-xs text-slate-500">
                      double click on any date to deslect them and leave it empty
                    </span>
                  )}
                  </div>
                </FormControl>
