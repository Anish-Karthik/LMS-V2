...
import { getUser, isTeacher, isUserPurchasedCourse } from '@/lib/actions/user.actions';
import { currentUser } from '@clerk/nextjs';
import Link from 'next/link';
import { redirect } from 'next/navigation';
import React from 'react'

const CourseDetailspage = async ({params}: {params: {courseId: string}}) => {
  ...
  return (
    <div>
      ...
      {userInfo.role === 'student' && 
        <CourseEnrollButton price={courseDetails?.price!} courseId={courseDetails?.id!}/>
      }
      {userInfo.role === 'teacher' && (isUserTeacher?(
        <Link href={"/teacher"}>
          <Button>
            Teacher dashboard
          </Button>
